...
int main() {
	...
	aes_key.assign(key1, key1 + 16);
	key_schedule(aes_key, round_keys);
	uint8_t exp_key1[176] = {
	...
	exp_round_keys.assign(exp_key1, exp_key1 + 176);
	...
	aes_key.clear();
	aes_key.assign(key2, key2 + 24);
	key_schedule(aes_key, round_keys);
	uint8_t exp_key2[208] = {
	...
	exp_round_keys.clear();
	exp_round_keys.assign(exp_key2, exp_key2 + 208);
	if (round_keys == exp_round_keys) {
	...
	aes_key.clear();
	aes_key.assign(key3, key3 + 32);
	key_schedule(aes_key, round_keys);
	uint8_t exp_key3[240] = {
		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
	...
	exp_round_keys.clear();
	exp_round_keys.assign(exp_key3, exp_key3 + 240);
	if (round_keys == exp_round_keys) {
