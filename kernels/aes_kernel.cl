/**
	Nella libreria Botan le chiavi sono passate come
	stato interno dell'oggetto. Su OpenCL questa cosa
	non � molto possibile in quanto non � presente una
	reale astrazione di classi/oggetti. 
	La struttura che ho pensato � quindi di creare le chiavi
	sul device (all'interno del main) tramite la libreria 
	botan, inserirle nel contesto di un safe_vector (un vettore
	con un allocator che azzera la memoria prima di uscire) e 
	passarle (come vettori di uchar(=byte)) al kernel come parametro.
	
*/

// Passo la macro del .cpp per rendere il codice pi� snello	
#if defined(AES_128)
    #define AES_KEY_SIZE 128
    #define NUM_ROUND 10

#elif defined(AES_192)
   #define AES_KEY_SIZE 192
   #define NUM_ROUND 12

#else
    #define AES_KEY_SIZE 256
    #define NUM_ROUND 14
#endif

__constant uchar SBox[256] = {
	
};

/**
* AES inverse sbox, calculated with affine transformation followed by the multiplication inverse
*/
__constant uchar Inv_SBox[256] = {
    
};

// Table to do faster multiplication in GF(2^8)
// 0x03 genarator
__constant uchar log_table[256] = {
    
};

__constant uchar alog_table[256] = {
    
};

inline uchar16 SubBytes(uchar16 s) {
    return (uchar16)(
        SBox[s.s0], SBox[s.s1], SBox[s.s2], SBox[s.s3],
        SBox[s.s4], SBox[s.s5], SBox[s.s6], SBox[s.s7],
        SBox[s.s8], SBox[s.s9], SBox[s.sa], SBox[s.sb],
        SBox[s.sc], SBox[s.sd], SBox[s.se], SBox[s.sf]
    );
}

inline uchar16 InvSubBytes(uchar16 s) {
    return (uchar16)(
        Inv_SBox[s.s0], Inv_SBox[s.s1], Inv_SBox[s.s2], Inv_SBox[s.s3],
        Inv_SBox[s.s4], Inv_SBox[s.s5], Inv_SBox[s.s6], Inv_SBox[s.s7],
        Inv_SBox[s.s8], Inv_SBox[s.s9], Inv_SBox[s.sa], Inv_SBox[s.sb],
        Inv_SBox[s.sc], Inv_SBox[s.sd], Inv_SBox[s.se], Inv_SBox[s.sf]
    );
}

inline uchar16 ShiftRows(uchar16 s) {
    return (uchar16)(
        s.s0, s.s1, s.s2, s.s3,
        s.s5, s.s6, s.s7, s.s4,
        s.sa, s.sb, s.s8, s.s9,
        s.sf, s.sc, s.sd, s.se
    );
}

inline uchar16 InvShiftRows(uchar16 s) {
    return (uchar16)(
        s.s0, s.s1, s.s2, s.s3,
        s.s7, s.s4, s.s5, s.s6,
        s.sa, s.sb, s.s8, s.s9,
        s.sd, s.se, s.sf, s.sc
    );
}

// Code taken from https://www.samiam.org/galois.html
inline uchar gmul(uchar a, uchar b) {
    int s;
	int q;
	int z = 0;
	s = log_table[a] + log_table[b];
	s %= 255;
	/* Get the antilog */
	s = alog_table[s];
	/* Now, we have some fancy code that returns 0 if either
           a or b are zero; we write the code this way so that the
           code will (hope  fully) run at a constant speed in order to
           minimize the risk of timing attacks */
	q = s;
	if(a == 0) {
		s = z;
	}
	else {
		s = q;
	}
	if(b == 0) {
		s = z;
	} 
	else {
		q = z;
	}
	return s;
}

// Mult by 2 in GF(2^8)
// To multiply by 3 you can just xor the same vector
inline uchar mult2GF(uchar a) {
    return (a << 1) ^ ((a >> 7) * 0x1b);
}

inline uchar4 MixOneColumn(uchar4 col) {
    uchar a0 = col.s0, a1 = col.s1, a2 = col.s2, a3 = col.s3;
    uchar4 res;
    res.s0 = mult2GF(a0) ^ (mult2GF(a1) ^ a1) ^ a2 ^ a3;
    res.s1 = a0 ^ mult2GF(a1) ^ (mult2GF(a2) ^ a2) ^ a3;
    res.s2 = a0 ^ a1 ^ mult2GF(a2) ^ (mult2GF(a3) ^ a3);
    res.s3 = (mult2GF(a0) ^ a0) ^ a1 ^ a2 ^ mult2GF(a3);

    return res;
}

inline uchar4 InvMixOneColumn(uchar4 col) {
    uchar a0 = col.s0, a1 = col.s1, a2 = col.s2, a3 = col.s3;
    uchar4 res;
    res.s0 = gmul(a0, 0x0e) ^ gmul(a1, 0x0b) ^ gmul(a2, 0x0d) ^ gmul(a3, 0x09);
    res.s1 = gmul(a0, 0x09) ^ gmul(a1, 0x0e) ^ gmul(a2, 0x0b) ^ gmul(a3, 0x0d);
    res.s2 = gmul(a0, 0x0d) ^ gmul(a1, 0x09) ^ gmul(a2, 0x0e) ^ gmul(a3, 0x0b);
    res.s3 = gmul(a0, 0x0b) ^ gmul(a1, 0x0d) ^ gmul(a2, 0x09) ^ gmul(a3, 0x0e);

    return res;
}

inline uchar16 MixColumns(uchar16 s) {
    uchar16 result;

    // Mix every columns separately
    uchar4 c0 = (uchar4) (s.s0, s.s4, s.s8, s.sc);
    c0 = MixOneColumn(c0);
    result.s0 = c0.s0; result.s4 = c0.s1; result.s8 = c0.s2; result.sc = c0.s3;

    uchar4 c1 = (uchar4)(s.s1, s.s5, s.s9, s.sd);
    c1 = MixOneColumn(c1);
    result.s1 = c1.s0; result.s5 = c1.s1; result.s9 = c1.s2; result.sd = c1.s3;

    uchar4 c2 = (uchar4)(s.s2, s.s6, s.sa, s.se);
    c2 = MixOneColumn(c2);
    result.s2 = c2.s0; result.s6 = c2.s1; result.sa = c2.s2; result.se = c2.s3;
    
    uchar4 c3 = (uchar4)(s.s3, s.s7, s.sb, s.sf);
    c3 = MixOneColumn(c3);
    result.s3 = c3.s0; result.s7 = c3.s1; result.sb = c3.s2; result.sf = c3.s3;

    return result;
}

inline uchar16 InvMixColumns(uchar16 s) {
    uchar16 result;

    // Mix every columns separately
    uchar4 c0 = (uchar4) (s.s0, s.s4, s.s8, s.sc);
    c0 = InvMixOneColumn(c0);
    result.s0 = c0.s0; result.s4 = c0.s1; result.s8 = c0.s2; result.sc = c0.s3;

    uchar4 c1 = (uchar4)(s.s1, s.s5, s.s9, s.sd);
    c1 = InvMixOneColumn(c1);
    result.s1 = c1.s0; result.s5 = c1.s1; result.s9 = c1.s2; result.sd = c1.s3;

    uchar4 c2 = (uchar4)(s.s2, s.s6, s.sa, s.se);
    c2 = InvMixOneColumn(c2);
    result.s2 = c2.s0; result.s6 = c2.s1; result.sa = c2.s2; result.se = c2.s3;
    
    uchar4 c3 = (uchar4)(s.s3, s.s7, s.sb, s.sf);
    c3 = InvMixOneColumn(c3);
    result.s3 = c3.s0; result.s7 = c3.s1; result.sb = c3.s2; result.sf = c3.s3;

    return result;
}

inline uchar16 AddRoundKey(uchar16 s, uchar16 key) {
    return s ^ key;
}

/** 
* @brief Encrypt with GCM AES
* @param input 16 byte block input
* @param output 16 byte block ciphered
* @param round_key 16 byte round key to cipher
*/
__kernel void encrypt_n(__global const uchar *input, __global uchar *output, __global const uchar *round_key) {
    // Useful to work with parallelism
    // It's better to use __private memory because is quicker
    int gid = get_global_id(0);
    //int offset = gid * 16;
    uchar16 state = vload16(0, input);

    // First round
    state = AddRoundKey(state, vload16(0, round_key));

    // 1 - num_round-1
    for (int i = 1; i < NUM_ROUND; ++i) {
        state = SubBytes(state);
        state = ShiftRows(state);
        state = MixColumns(state);
        state = AddRoundKey(state, vload16(i, round_key));
    }

    // Last Round
    state = SubBytes(state);
    state = ShiftRows(state);
    state = AddRoundKey(state, vload16(NUM_ROUND, round_key));

    /*
    printf("Output opencl: %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u\n",
       state.s0, state.s1, state.s2, state.s3,
       state.s4, state.s5, state.s6, state.s7,
       state.s8, state.s9, state.sa, state.sb,
       state.sc, state.sd, state.se, state.sf); */
    // Write the result
    vstore16(state, 0, output);
}


__kernel void decrypt_n(__global uchar *input, __global uchar *output, __global uchar *round_key) {
    int gid = get_global_id(0);
    //int offset = gid * 16;
    uchar16 state = vload16(gid, input);

    // First Round
    state = AddRoundKey(state, vload16(NUM_ROUND, round_key));
    state = InvShiftRows(state);
    state = InvShiftRows(state);

    // 1 - num_round-1
    for (int i = NUM_ROUND - 1; i > 0; --i) {
        state = AddRoundKey(state, vload16(i, round_key));
        state = InvMixColumns(state);
        state = InvShiftRows(state);
        state = InvSubBytes(state);        
    }

    // Last Round
    state = AddRoundKey(state, vload16(0, round_key));

    // Write the result
    vstore16(state, 0, output);
}