...
AESOpenCL::AESOpenCL() {
	...
	cl_int err_pr = _program.build({ _device }, aes_version_define.c_str());
	...
	_round_keys_buf = cl::Buffer(_context, CL_MEM_READ_ONLY, _max_round_key_size);
...
void AESOpenCL::aes_encrypt(std::span<const uint8_t> plain_text, std::span<uint8_t> cipher_text, 
	...
	// Copy in preallocated buffer
	_queue.enqueueWriteBuffer(_round_keys_buf, CL_TRUE, 0, round_keys.size(), round_keys.data());
	_queue.enqueueWriteBuffer(_plaintext_buf, CL_TRUE, 0, plain_text.size(), plain_text.data());

	// Kernel functor
	cl::KernelFunctor<cl::Buffer, cl::Buffer, cl::Buffer, cl_uint> encrypter(_encrypt_kernel);
	...
	encrypter(cl::EnqueueArgs(_queue, cl::NDRange(global_work_size)),
		_plaintext_buf, _ciphertext_buf, _round_keys_buf, n_blocks);
	// Read data
	...
	_queue.enqueueWriteBuffer(_round_keys_buf, CL_FALSE, 0, _max_round_key_size, zero_buf.data());
}
...
void AESOpenCL::aes_decrypt(std::span<const uint8_t> cipher_text, std::span<uint8_t> plain_text,
	...
	cl::KernelFunctor<cl::Buffer, cl::Buffer, cl::Buffer, cl_uint> decypter(_decrypt_kernel);
	size_t global_work_size = num_blocks; 
	cl_uint n_blocks = num_blocks;
	decypter(cl::EnqueueArgs(_queue, cl::NDRange(global_work_size)), _ciphertext_buf, _plaintext_buf, _round_keys_buf, n_blocks);
...
cl::Buffer& AESOpenCL::get_round_keys_buffer() { return _round_keys_buf; }
	...
	std::span<const uint8_t> round_keys, std::span<const uint8_t> iv) 
{
	...
	_aes.get_queue().enqueueWriteBuffer(_aes.get_round_keys_buffer(), CL_TRUE, 0, round_keys.size(), round_keys.data());
	...
	cl::KernelFunctor<cl::Buffer, cl::Buffer, cl::Buffer, cl::Buffer, cl_uint> encrypter(_encrypt_ctr_kernel);

	size_t global_work_size = num_blocks; // parallel work 1 work group for 1 aes block
	cl_uint n_blocks = num_blocks;
	//std::cout << "Num WG: " << global_work_size << std::endl;
	encrypter(cl::EnqueueArgs(_aes.get_queue(), cl::NDRange(global_work_size)),
		_aes.get_plaintext_buffer(), _aes.get_ciphertext_buffer(), _aes.get_round_keys_buffer(), _iv_buf, n_blocks);
	// Read data
	...
	_aes.get_queue().enqueueWriteBuffer(_aes.get_round_keys_buffer(), CL_FALSE, 0, _max_round_key_size, zero_buf.data());
