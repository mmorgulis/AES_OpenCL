...
public:
	...
	std::array<uint8_t, 16> compute_ghash(std::span<uint8_t> cipher_text);
	std::array<uint8_t, 16> compute_tag();
	void precompute_tables();
	std::array<uint8_t, 16> multGF128(std::array<uint8_t, 16> x);
	std::array<uint8_t, 16> multGF128_plain(std::array<uint8_t, 16> X) const;
};
