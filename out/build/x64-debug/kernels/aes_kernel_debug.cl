__constant uchar SBox[256] = {
	
};

/**
* AES inverse sbox, calculated with affine transformation followed by the multiplication inverse
*/
__constant uchar Inv_SBox[256] = {
    
};

// Table to do faster multiplication in GF(2^8)
// 0x03 genarator
__constant uchar log_table[256] = {
    
};

__constant uchar alog_table[256] = {
    
};

inline uchar16 SubBytes(uchar16 s) {
    return (uchar16)(
        SBox[s.s0], SBox[s.s1], SBox[s.s2], SBox[s.s3],
        SBox[s.s4], SBox[s.s5], SBox[s.s6], SBox[s.s7],
        SBox[s.s8], SBox[s.s9], SBox[s.sa], SBox[s.sb],
        SBox[s.sc], SBox[s.sd], SBox[s.se], SBox[s.sf]
    );
}

inline uchar16 InvSubBytes(uchar16 s) {
    return (uchar16)(
        Inv_SBox[s.s0], Inv_SBox[s.s1], Inv_SBox[s.s2], Inv_SBox[s.s3],
        Inv_SBox[s.s4], Inv_SBox[s.s5], Inv_SBox[s.s6], Inv_SBox[s.s7],
        Inv_SBox[s.s8], Inv_SBox[s.s9], Inv_SBox[s.sa], Inv_SBox[s.sb],
        Inv_SBox[s.sc], Inv_SBox[s.sd], Inv_SBox[s.se], Inv_SBox[s.sf]
    );
}

// Row-major version
inline uchar16 ShiftRows(uchar16 s) {
    return (uchar16)(
        s.s0, s.s5, s.sa, s.sf,
        s.s4, s.s9, s.se, s.s3,
        s.s8, s.sd, s.s2, s.s7,
        s.sc, s.s1, s.s6, s.sb
    );
}

inline uchar16 InvShiftRows(uchar16 s) {
    return (uchar16)(
        s.s0, s.sd, s.sa, s.s7,
        s.s4, s.s1, s.se, s.sb,
        s.s8, s.s5, s.s2, s.sf,
        s.sc, s.s9, s.s6, s.s3  
    );
}

// Code taken from https://www.samiam.org/galois.html
// General multiplication in GF(2^8)
// The sum is only the xor
inline uchar mult_gf8(uchar a, uchar b) {
    int s;
	int q;
	int z = 0;
	s = log_table[a] + log_table[b];
	s %= 255;
	/* Get the antilog */
	s = alog_table[s];
	/* Now, we have some fancy code that returns 0 if either
           a or b are zero; we write the code this way so that the
           code will (hope  fully) run at a constant speed in order to
           minimize the risk of timing attacks */
	q = s;
	if(a == 0) {
		s = z;
	}
	else {
		s = q;
	}
	if(b == 0) {
		s = z;
	} 
	else {
		q = z;
	}
	return s;
}

// Mult by 2 in GF(2^8)
// To multiply by 3 you can just xor the same vector
inline uchar mult2_gf8(uchar a) {
    return (a << 1) ^ ((a >> 7) * 0x1b);
}

inline uchar4 MixOneColumn(uchar4 col) {
    uchar a0 = col.s0, a1 = col.s1, a2 = col.s2, a3 = col.s3;
    uchar4 res;
    res.s0 = mult2_gf8(a0) ^ (mult2_gf8(a1) ^ a1) ^ a2 ^ a3;
    res.s1 = a0 ^ mult2_gf8(a1) ^ (mult2_gf8(a2) ^ a2) ^ a3;
    res.s2 = a0 ^ a1 ^ mult2_gf8(a2) ^ (mult2_gf8(a3) ^ a3);
    res.s3 = (mult2_gf8(a0) ^ a0) ^ a1 ^ a2 ^ mult2_gf8(a3);

    return res;
}

inline uchar4 InvMixOneColumn(uchar4 col) {
    uchar a0 = col.s0, a1 = col.s1, a2 = col.s2, a3 = col.s3;
    uchar4 res;
    res.s0 = mult_gf8(a0, 0x0e) ^ mult_gf8(a1, 0x0b) ^ mult_gf8(a2, 0x0d) ^ mult_gf8(a3, 0x09);
    res.s1 = mult_gf8(a0, 0x09) ^ mult_gf8(a1, 0x0e) ^ mult_gf8(a2, 0x0b) ^ mult_gf8(a3, 0x0d);
    res.s2 = mult_gf8(a0, 0x0d) ^ mult_gf8(a1, 0x09) ^ mult_gf8(a2, 0x0e) ^ mult_gf8(a3, 0x0b);
    res.s3 = mult_gf8(a0, 0x0b) ^ mult_gf8(a1, 0x0d) ^ mult_gf8(a2, 0x09) ^ mult_gf8(a3, 0x0e);

    return res;
}

inline uchar16 MixColumns(uchar16 s) {
    uchar16 result;

    // Mix every columns separately
    uchar4 c0 = (uchar4) (s.s0, s.s1, s.s2, s.s3);
    c0 = MixOneColumn(c0);
    result.s0 = c0.s0; result.s1 = c0.s1; result.s2 = c0.s2; result.s3 = c0.s3;

    uchar4 c1 = (uchar4)(s.s4, s.s5, s.s6, s.s7);
    c1 = MixOneColumn(c1);
    result.s4 = c1.s0; result.s5 = c1.s1; result.s6 = c1.s2; result.s7 = c1.s3;

    uchar4 c2 = (uchar4)(s.s8, s.s9, s.sa, s.sb);
    c2 = MixOneColumn(c2);
    result.s8 = c2.s0; result.s9 = c2.s1; result.sa = c2.s2; result.sb = c2.s3;
    
    uchar4 c3 = (uchar4)(s.sc, s.sd, s.se, s.sf);
    c3 = MixOneColumn(c3);
    result.sc = c3.s0; result.sd = c3.s1; result.se = c3.s2; result.sf = c3.s3;

    return result;
}

inline uchar16 InvMixColumns(uchar16 s) {
    uchar16 result;

    // Mix every columns separately
    uchar4 c0 = (uchar4) (s.s0, s.s1, s.s2, s.s3);
    c0 = InvMixOneColumn(c0);
    result.s0 = c0.s0; result.s1 = c0.s1; result.s2 = c0.s2; result.s3 = c0.s3;

    uchar4 c1 = (uchar4)(s.s4, s.s5, s.s6, s.s7);
    c1 = InvMixOneColumn(c1);
    result.s4 = c1.s0; result.s5 = c1.s1; result.s6 = c1.s2; result.s7 = c1.s3;

    uchar4 c2 = (uchar4)(s.s8, s.s9, s.sa, s.sb);
    c2 = InvMixOneColumn(c2);
    result.s8 = c2.s0; result.s9 = c2.s1; result.sa = c2.s2; result.sb = c2.s3;
    
    uchar4 c3 = (uchar4)(s.sc, s.sd, s.se, s.sf);
    c3 = InvMixOneColumn(c3);
    result.sc = c3.s0; result.sd = c3.s1; result.se = c3.s2; result.sf = c3.s3;

    return result;
}

inline uchar16 AddRoundKey(uchar16 s, uchar16 key) {
    //return s ^ key;
    uchar16 result = s ^ key;

    // Usa un array temporaneo per poter iterare
    uchar* ps = (uchar*)&s;
    uchar* pk = (uchar*)&key;
    uchar* pr = (uchar*)&result;

    printf("AddRoundKey xor:\n");
    for (int i = 0; i < 16; ++i) {
        printf("  s[%2d] = 0x%02x, key[%2d] = 0x%02x, xor = 0x%02x\n",
               i, ps[i], i, pk[i], pr[i]);
    }

    return result;
}

inline uchar16 encrypt_block(uchar16 input, __constant const uchar *round_keys, uint NUM_ROUND) {

    printf("RoundKey:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           round_keys[0], round_keys[1], round_keys[2], round_keys[3],
           round_keys[4], round_keys[5], round_keys[6], round_keys[7],
           round_keys[8], round_keys[9], round_keys[10], round_keys[11],
           round_keys[12], round_keys[13], round_keys[14], round_keys[15]);
    printf("Input initial:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);

    // First round
    input = AddRoundKey(input, vload16(0, round_keys));
    printf("After addroundkey:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);

    for (int i = 1; i < NUM_ROUND; ++i) {
        input = SubBytes(input);
        printf("After subbytes:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
        input = ShiftRows(input);
        printf("After shiftrows:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
        input = MixColumns(input);
        printf("After mixcolums:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
        input = AddRoundKey(input, vload16(i, round_keys));
        printf("After addroundkey:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
    }

    // Last Round
    input = SubBytes(input);
    printf("After subbytes:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
    input = ShiftRows(input);
    printf("After shiftrows:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
    input = AddRoundKey(input, vload16(NUM_ROUND, round_keys));
    printf("After addroundkey:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           input[0], input[1], input[2], input[3],
           input[4], input[5], input[6], input[7],
           input[8], input[9], input[10], input[11],
           input[12], input[13], input[14], input[15]);
   
    // return in row major
    uchar16 result = (uchar16)
        (input.s0, input.s4, input.s8, input.sc,
        input.s1, input.s5, input.s9, input.sd,
        input.s2, input.s6, input.sa, input.se,
        input.s3, input.s7, input.sb, input.sf);
    return result;

}

inline uchar16 decrypt_block(uchar16 input, __constant const uchar *round_keys, uint NUM_ROUND) {
     // First Round
    input = AddRoundKey(input, vload16(NUM_ROUND, round_keys));

    // num_round-1 to 1
    for (int i = NUM_ROUND - 1; i > 0; --i) {
        input = InvShiftRows(input);
        input = InvSubBytes(input);
        input = AddRoundKey(input, vload16(i, round_keys));
        input = InvMixColumns(input);        
    }

    // Last Round
    input = InvShiftRows(input);
    input = InvSubBytes(input);
    input = AddRoundKey(input, vload16(0, round_keys));

    return input;
}

/** 
* @brief Encrypt AES
* @param input plain text n blocks
* @param output cipher text n blocks
* @param round_key expansion of key
*/
__kernel void encrypt_n(__global const uchar *input, __global uchar *output, __constant const uchar *round_keys, 
        uint num_blocks, uint NUM_ROUND) {
    int gid = get_global_id(0);
    if (gid >= num_blocks) return;

    uchar16 state = vload16(gid, input);
    uchar16 out_block = encrypt_block(state, round_keys, NUM_ROUND);
    vstore16(out_block, 0, output + gid * 16);

}


__kernel void decrypt_n(__global const uchar *input, __global uchar *output, __constant uchar *round_keys, 
        uint num_blocks, uint NUM_ROUND) {
    int gid = get_global_id(0);
    if (gid >= num_blocks) return;

    uchar16 state = vload16(gid, input);
    uchar16 out_block = decrypt_block(state, round_keys, NUM_ROUND);
    vstore16(out_block, 0, output + gid * 16);
   
}

// Increment and return counter with gid
inline uchar16 make_counter(__constant const uchar *iv, uint block_index) {
    uchar16 counter;
    for (uint i = 0; i < 12; ++i)
        counter[i] = iv[i];

    counter[12] = (block_index >> 24) & 0xFF;
    counter[13] = (block_index >> 16) & 0xFF;
    counter[14] = (block_index >> 8) & 0xFF;
    counter[15] = block_index & 0xFF;

    // Row major format
    return counter;

}

inline uchar16 transpose_block(uchar16 in) {
    return (uchar16)(
        in.s0, in.s4, in.s8, in.sc,
        in.s1, in.s5, in.s9, in.sd,
        in.s2, in.s6, in.sa, in.se,
        in.s3, in.s7, in.sb, in.sf
    );
}

__kernel void encrypt_ctr(__global const uchar *input, __global uchar *output, __constant const uchar *round_keys, 
                        __constant const uchar *iv, uint num_blocks, uint NUM_ROUND) {
    int gid = get_global_id(0);
    if (gid >= num_blocks) return;

    // Counter
    uchar16 counter = make_counter(iv, gid + 1); // counter parte da 01
    printf("Counter:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           counter[0], counter[1], counter[2], counter[3],
           counter[4], counter[5], counter[6], counter[7],
           counter[8], counter[9], counter[10], counter[11],
           counter[12], counter[13], counter[14], counter[15]);

    counter = encrypt_block(counter, round_keys, NUM_ROUND);

    // XOR
    uchar16 plain_text = vload16(0, input + gid * 16);
    printf("Plaint:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           plain_text[0], plain_text[1], plain_text[2], plain_text[3],
           plain_text[4], plain_text[5], plain_text[6], plain_text[7],
           plain_text[8], plain_text[9], plain_text[10], plain_text[11],
           plain_text[12], plain_text[13], plain_text[14], plain_text[15]);
    printf("Xor Counter:\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n%02x %02x %02x %02x\n",
           counter[0], counter[1], counter[2], counter[3],
           counter[4], counter[5], counter[6], counter[7],
           counter[8], counter[9], counter[10], counter[11],
           counter[12], counter[13], counter[14], counter[15]);
    printf("\n\n\n");
    counter = plain_text ^ counter;

    vstore16(counter, 0, output + gid * 16);
}


